...
for i = countyid
    ...
    MaxTime = days(datetime(2021,02,31) - timeRef(1)); % length(timeRef)+sim_time;
    ...
    % set all five dat2es to be later than the simulation period (ex. set the
    % year out in the future to 2022). The amount of lockdown release at each
    % date is set in the function diff_eqn1.m.
    lockdown_release_1 = timeRef(end); % this should stay fixed now since they've started lifting lockdown in FL
    ...
    soc_dist_end_date = timeRef(end); %datetime(2020, 07, 24);
    ...
    % Get movement data
    % movement_data = get_movement_data(Location_arr, timeRef);
    ...
    save(sprintf('%s_Scen1.mat',county));
